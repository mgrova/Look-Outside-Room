...
# Taken from: https://github.com/yolish/multi-scene-pose-transformer/blob/main/models/pose_losses.py
...
class CameraPoseLoss(nn.Module):
    ...
    def __init__(self):
        ...
        # TODO. Define this parameters from config file
    ...
    def forward(self, est_pose, gt_pose):
            ...
            """
            # # Position loss
            # l_x = torch.norm(gt_pose[0:3] - est_pose[0:3], p=self.norm).mean()
            # # Orientation loss (normalized to unit norm)
            # l_q = torch.norm(F.normalize(gt_pose[3:], p=2, dim=0) - F.normalize(est_pose[3:], p=2, dim=0), p=self.norm).mean()
            # Position loss
